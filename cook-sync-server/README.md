...
| `FIREBASE_SERVICE_ACCOUNT_JSON` | No at startup (defaults to blank, disabling push) — needed in practice for push notifications/system-announcement broadcasts to deliver | **Base64-encoded** JSON contents of a Firebase service-account key (Firebase console > Project settings > Service accounts > Generate new private key) — `base64 -i service-account.json \| tr -d '\n'`. Must be base64, not raw JSON: the local `.env` file is loaded as a `.properties` file, whose parser silently unescapes the `\n` sequences inside the key's `private_key` field, corrupting it |
| `TRANSLATION_CONTACT_EMAIL` | No (defaults to blank — `MyMemoryTranslationProvider` still works anonymously) | Free-tier contact email for the [MyMemory Translation API](https://mymemory.translated.net) (auto-translation between Hebrew/English content). No signup — any address works. Raises MyMemory's free daily quota from 5,000 to 50,000 characters; leave blank to use MyMemory anonymously at the lower quota |
| `GEMINI_API_KEY` | No at startup (defaults to blank) — needed in practice for smart recipe import (URL/photo → structured recipe) to work; a blank key fails each import job with a "GEMINI_API_KEY is not set" error rather than blocking startup | Google AI Studio API key for the Gemini API's free tier (no billing account required) — used by `GeminiRecipeExtractionProvider` to extract structured recipe data from a fetched web page or submitted photo(s). Get one at [aistudio.google.com/apikey](https://aistudio.google.com/apikey) |

...
/**
 * Composed Bean Validation constraint for a re-authentication {@code currentPassword} field,
 * shared by every request DTO that requires the caller to prove their current password before
 * a sensitive account change (password change, email change, account deletion). Delegates
 * entirely to the meta-annotated {@link NotBlank} and {@link Size} constraints rather than a
 * dedicated validator.
 *
 * @author Yaron Serlin
 * @version 1.0
